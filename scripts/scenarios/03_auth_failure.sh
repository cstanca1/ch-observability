...
t = re.sub(r'(^\s+INCIDENT_MODE:\s*")[^"]+(")', r'\1auth_failure\2', t, count=1, flags=re.M)

p.write_text(t)
print("INCIDENT_MODE set to auth_failure")
PY
}
...
echo
echo "=================================================="
echo " Scenario 3: Authentication Failure"
echo "=================================================="
echo
echo "What this scenario does:"
echo "  - Ensures kafka, clickhouse, and grafana are running"
echo "  - Sets INCIDENT_MODE to auth_failure"
echo "  - Restarts the generator"
echo "  - Produces auth-related failures and downstream pressure"
echo
echo "What to show:"
echo "  - Grafana Error Analysis dashboard"
echo "  - Grafana Service Health dashboard"
echo "  - Auth error spikes"
echo "  - Status codes like 401 / 403 / 429 / 500"
echo "  - Downstream impact on checkout"
echo
...
echo "Restarting generator for authentication failure scenario..."
docker compose up -d --build generator >/dev/null
echo "Generator restarted."
echo

echo "Scenario 3 is now active."
...
echo "  service,"
echo "  status_code,"
echo "  count() AS events"
echo "FROM logs"
echo "WHERE level = 'ERROR'"
echo "GROUP BY service, status_code"
echo "ORDER BY events DESC;"
echo
